...
function App() {
  ...
  const addToHistory = (method, url) => {
    setHistory((prevHistory) => [{ method, url }, ...prevHistory]);
  };

  const handleSelectHistory = (selectedItem) => {
    // Handle the selection of a history item
    setHttpMethod(selectedItem.method);
    setApiEndpoint(selectedItem.url);
  };
  ...
  const handleApiSubmit = async () => {
    try {
      ...
      addToHistory(httpMethod, apiEndpoint);
      if (!apiEndpoint) {
        setResponse(null);
  ...
  return (
    <div style={{ position: "relative", minHeight: "100vh", display: "grid", gridTemplateColumns: "0fr auto" }}>
      <SideMenu history={history} onSelectHistory={handleSelectHistory} />      <div style={{ padding: 16 }}>
        <Select
          defaultValue="GET"
